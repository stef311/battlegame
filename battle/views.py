...
def level_up_building(user, building):	
	...
	if building.name == "market" and building.level <= 2:
		user.belongings.market += 1
		user.belongings.save()
	elif building.name == "army" and building.level <= 2:
		user.belongings.army += 1
		user.belongings.save()
	elif building.name == "warehouse" and building.level <= 2:
		user.belongings.warehouse += 1
		user.belongings.save()
	elif building.name == "museum" and building.level <= 2:
		user.belongings.museum += 1
		user.belongings.save()
	elif building.name == "academy" and building.level <= 2:
		user.belongings.academy += 1
		user.belongings.save()
...
def central(request):
	...
	context["army"] = user.belongings.army
	context["market"] = user.belongings.market
	context["academy"] = user.belongings.academy
	context["warehouse"] = user.belongings.warehouse
	context["museum"] = user.belongings.museum
...
def market(request):
	...
	if user.belongings.market > 0:
		...
		return HttpResponse("You do not have an army yet")
...
@login_required
def upgrade_building(request, building_name ,building_level):
	user = request.user
	context = {}
	print(building_name)
	building = Building.objects.get(name=building_name, level=building_level)
	if building:
		print "BUILDING!"
		name = building.name
...
@login_required
def upgrade_building_done(request, building_name, building_level):
	user = request.user
	building = Building.objects.get(name=building_name,level=building_level)
